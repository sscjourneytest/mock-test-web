...
function _filterCacheKey() {
    // Unique key per exam page so CGL filters don't bleed into MTS etc.
...
function loadSavedFilters() {
    try {
        ...
        if (saved) {
            ...
            // Merge so any new keys in the default object are still present
...
function setupFilters(years) {
    ...
    // Restore active type pill to match saved filter
    ...
    // If saved filter was sectional, restore section-wrap visibility
...
function renderSectionPills() {
    ...
    // Use saved section if valid, otherwise default to first
