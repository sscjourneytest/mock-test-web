...
async function initExamEngine() {
    // Restore last-used filters before anything else renders
    loadSavedFilters();
    const pathParts = window.location.pathname.split('/');
    ...
    try {
        ...
        // Only set year default if no saved filter, or saved year no longer exists in data
        if (!currentFilters.year || !years.includes(currentFilters.year)) {
            currentFilters.year = years.length > 0 ? years.sort().reverse()[0] : "default";
        }

        // Validate saved subject still exists under the restored year
        ...
        // Validate saved topic still exists under the restored subject
...
async function syncWithCloud(examName) {
    ...
    try {
        ...
        // freshData = { "IMPS-001": true, "IMPS-002": true }
...
function renderMocks() {
    ...
    // Update ONLY this mapping logic
    itemsToDisplay = itemsToDisplay.map(item => {
        // Use linkStr if it exists, otherwise fallback to standard id
        const finalParam = item.linkStr ? `id=${item.linkStr}` : `id=${item.id}`;
        return { ...item, linkParam: finalParam, originalId: item.id };
    });
